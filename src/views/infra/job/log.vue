<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="120px">
      <el-form-item label="处理器的名字" prop="executorHandler">
        <el-input v-model="queryParams.executorHandler" placeholder="请输入处理器的名字" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="任务状态" prop="alarmStatus">
        <el-select v-model="queryParams.alarmStatus" placeholder="请选择任务状态" clearable>
    ...
    <el-table v-loading="loading" :data="list" v-if="list.length!=0">
      ...
      <el-table-column label="处理器的名字" align="center" prop="executorHandler" />
      ...
      <el-table-column label="任务状态" align="center" prop="alarmStatus">
        <template v-slot="scope">
          <dict-tag :type="DICT_TYPE.INFRA_JOB_LOG_STATUS" :value="scope.row.alarmStatus" />
        </template>
      </el-table-column>
    ...
    <pagination v-if="list.length!=0" :total="total" :page.sync="queryParams.pageNo" :limit.sync="queryParams.pageSize"
                @pagination="getList"/>
    <el-empty
      v-if="list.length==0"
      ...
      <el-form ref="form" :model="form" label-width="120px" size="mini">
        <el-row>
          <el-col :span="24">
            ...
            <el-form-item label="处理器的名字：">{{ form.executorHandler }}</el-form-item>
            <el-form-item label="处理器的参数：">{{ form.executorParam }}</el-form-item>
            <el-form-item label="执行时间：">{{ parseTime(form.handleTime) }}</el-form-item>
            <el-form-item label="任务状态：">
              <dict-tag :type="DICT_TYPE.INFRA_JOB_LOG_STATUS" :value="form.alarmStatus" />
            </el-form-item>
            <el-form-item label="执行结果："><span v-html="form.triggerMsg "></span></el-form-item>
          </el-col>
        </el-row>
      </el-form>
