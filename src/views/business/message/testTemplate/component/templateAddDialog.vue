    ...
    >
          ...
          <div v-if="active == 1">
            ...
            <div v-if="dataForm.channelType == 3">
              ...
              <el-form-item label="抄送人:">
                ...
                <el-button v-else class="button-new-tag" size="small" @click="showInputToCC">新 增</el-button>
              </el-form-item>
            </div>
          </div>
        </el-form>
      ...
      <div v-if="active == 2" style="margin: 10px 20px; text-align: center">
        <el-table
          :data="templateParam"
          style="width: 100%">
          <el-table-column label="模板参数" width="150" align="center">
          ...
          <el-table-column v-if="type != 'view'" width="120" align="center">
            <template slot-scope="scope">
          ...
          <div slot="append" v-if="type != 'view' && templateParam.length == 0">
            <el-button size="mini" type="primary" icon="el-icon-plus" plain @click="handleAddParam"></el-button>
          </div>
        </el-table>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleInputConfirm() {
      ...
      this.dataForm.userIds = this.dynamicTags.toString()
    },
    handleCloseToCC(tag) {
    ...
    handleDeleteParam(row) {
      const data = this.templateParam.filter(item => !(row.id === item.id))
    ...
    active2(){
      ...
      /*     if (this.type == 'add') {
          ...
          //由于站内信和邮件的模板是在平台配置的，所以此处需要对邮件和站内信做参数提取的处理，用于重新构建第三步的表格数据
          //短信和电话的模板内容是在第三方服务平台配置，所以此处不需要重新提取，只需要按之前配置的参数列表回显即可，回显逻辑已在打开这个页面时执行，此处不用处理
    ...
    editTemplate(data){
      ...
      if (this.dataForm.relatedParameter.templateParam != null || this.dataForm.relatedParameter.templateParam != undefined) {
        this.templateParam = [];
        Object.keys(this.dataForm.relatedParameter.templateParam).forEach(item => {
          var i = 1;
          var dataId = Date.now().toString(36) + Math.random().toString(36).substr(2, 9) + i;
          var data = {
            id: dataId + i,
            key: item,
            value: this.dataForm.relatedParameter.templateParam[item],
          };
